...
7. **Hand-off.** The skill prints the kickoff prompt as a fenced code block and tries to copy it to the system clipboard via `pbcopy`, `wl-copy`, `xclip`, `xsel`, `clip.exe`, or `clip` (whichever is available). The trailing line says whether the copy succeeded.
