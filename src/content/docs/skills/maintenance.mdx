...
Discovers outdated packages with `pnpm outdated --json`, audits `pnpm.overrides` for entries that are no longer needed, applies migrations for major version bumps, resolves dependency conflicts, and runs the full quality gate (`pnpm typecheck`, `pnpm lint`, `pnpm test --run`, `pnpm pw`, `pnpm build`).
