...
5. Runs the three-way merge per file directly (no CLI subcommand). Owned files are overwritten or conflict-patched; shared and wiki-owned files are merged or conflict-patched. `package.json`, `pnpm-workspace.yaml`, and `.gaia/audit-ci.yml` are all classed `shared` but receive field-aware key-level merges rather than whole-file patches (see below).
