...
Phase B remote-integration primitives invoked by `/setup-gaia-ci`. The slash command shells out to these for remote detection, the admin permission probe, token piping (stdin only), `workflow_dispatch` verification, and the `setup_complete` flip.
